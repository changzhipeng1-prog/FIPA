...
parser.add_argument(
    ...
    type=float,
    default=1e-2,
    help="Positive coefficient C in tau_m^(k) = ceil(log(C / (k * ||g_m^(k)||)) / log(rho_m^(k))).",
)
parser.add_argument("--time_budget", type=float, default=50.0)
parser.add_argument("--out_root", type=str, default="Results_adaptive")
...
# -------------------- 输出目录 --------------------
run_name = (
    f"adaptive_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
    f"_cap-{LOCAL_STEPS_CAP}_adapt-{C_COEF}_tau-{TAU_COEF}_step{STEP_SIZE}"
)
...
for r in range(1, MAX_ROUNDS + 1):
    ...
    # Correct formula: tau_m^(k) >= ceil(log(C / (k * ||g_m^(k)||)) / log(rho_m^(k)))
    # where rho_m^(k) = 1 - eta * lambda_min, and log(rho) < 0
    ...
    log_rho = float(np.log(decay))  # log(rho) = log(1 - eta * lambda_min) < 0
    if not np.isfinite(log_rho) or log_rho >= 0.0:
        if lambda_min_last is not None:
            lambda_min_safe = lambda_min_last
            decay = 1.0 - LR * lambda_min_safe
            log_rho = float(np.log(decay))
        else:
            raise RuntimeError(
                f"Non-negative log rho {log_rho} with decay={decay} at round {r} on rank {rank}"
            )

    # Compute: tau = log(C / (k * ||g||)) / log(rho)
    # Since log(rho) < 0, we rewrite as: tau = (log(C) - log(k) - log(||g||)) / log(rho)
    k_round = max(1, r)
    g_norm_safe = max(g_norm, 1e-12)  # Avoid log(0)
    
    log_numerator = np.log(TAU_COEF) - np.log(float(k_round)) - np.log(g_norm_safe)
    steps_est = log_numerator / log_rho
    ...
    if not np.isfinite(steps_est) or steps_est <= 0.0:
        if lambda_min_last is not None:
            ...
            raise RuntimeError(
                f"Non-positive tau estimate {steps_est} derived from log_rho={log_rho} at round {r}"
            )
