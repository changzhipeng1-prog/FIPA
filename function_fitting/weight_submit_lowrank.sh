...
# 输出路径包含客户端数量和N_FREQ
OUT_ROOT="${OUT_ROOT:-Results_lowrank_N${N_CLIENTS}_nf${N_FREQ}}"
...
for EPOCHS in $EPOCHS_LIST; do
  for STEP in $STEP_LIST; do
    for K in $TOP_K_LIST; do
          ...
          CFG_FILE="$CFG_DIR/${ALG_TAG}_cfg_arch_${ARCH_TAG}_N${N_CLIENTS}_E${EDGE_HASH}_nf${N_FREQ}_epochs${EPOCHS}_step${STEP}_k${K}_rmax${MAX_ROUNDS}_tb${TIME_BUDGET}s_seed${SEED}_${STAMP}.json"
...
PY
          ...
          SBATCH_ARGS=(
            ...
            --mem="${MEM}"
            -t "${TIME_LIMIT}"
            -J "${ALG_TAG}_1n${N_CLIENTS}_g${REQUEST_GPUS}_epochs${EPOCHS}_step${STEP}_k${K}_tb${TIME_BUDGET}s"
            -p "${PARTITION}"
            -o "slurm-%A.out"
          )
