#!/bin/bash
# 写配置(JSON) -> 串行 sbatch 多个单节点作业 -> 作业内用 mpirun 起 N_CLIENTS 进程（fullrank + fixed local training）
set -euo pipefail
...
# 可选
SEED="${SEED:-0}"
ALG_TAG="${ALG_TAG:-fullrank-fixed}"
...
# 输出路径包含客户端数量和N_FREQ
OUT_ROOT="${OUT_ROOT:-Results_fullrank_N${N_CLIENTS}_nf${N_FREQ}}"
...
for EPOCHS in $EPOCHS_LIST; do
  for STEP in $STEP_LIST; do
          ...
          CFG_FILE="$CFG_DIR/${ALG_TAG}_cfg_arch_${ARCH_TAG}_N${N_CLIENTS}_E${EDGE_HASH}_nf${N_FREQ}_epochs${EPOCHS}_step${STEP}_rmax${MAX_ROUNDS}_tb${TIME_BUDGET}s_seed${SEED}_${STAMP}.json"
...
PY
          ...
          SBATCH_ARGS=(
            ...
            --mem="${MEM}"
            -t "${TIME_LIMIT}"
            -J "${ALG_TAG}_1n${N_CLIENTS}_g${REQUEST_GPUS}_epochs${EPOCHS}_step${STEP}_tb${TIME_BUDGET}s"
            -p "${PARTITION}"
            -o "slurm-%A.out"
          )
          ...
          sbatch "${SBATCH_ARGS[@]}" --export="$EXPORTS" weight_run_fullrank.sh
