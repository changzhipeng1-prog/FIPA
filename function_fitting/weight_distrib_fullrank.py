# -*- coding: utf-8 -*-
"""Distributed full-rank matrix-weighted FL with fixed local training steps."""
...
parser.add_argument("--out_root", type=str, default="Results_fullrank_fixed")
...
if rank == 0:
    print(f"[config] lr={LR}, local_epochs={LOCAL_EPOCHS}, step_size={STEP_SIZE}, pinv_tol={PINV_TOL}")

# -------------------- 输出目录 --------------------
run_name = (
    f"fullrank_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
    f"_epochs{LOCAL_EPOCHS}_step{STEP_SIZE}"
)
...
if not os.path.exists(local_metrics_path):
    with open(local_metrics_path, "w") as f:
        f.write("round,g_norm,r_norm,local_epochs\n")
...
if rank == 0:
    ...
    with open(os.path.join(out_dir, "timings.csv"), "w") as f:
        f.write("round,tJJ_max,tLocal_max,tComm_max,tRecon,tPinv,tUpdate,tBcast_max,tRound,comm_total,local_total,mwfl_total\n")
...
for r in range(1, MAX_ROUNDS + 1):
    ...
    # ---- time: gather full-rank Hessian and delta ----
    t_c0 = time.perf_counter()
    H_host = np.asarray(device_get(JTJ))
    delta_host = np.asarray(device_get(delta_i))
    n_local = int(xi.shape[0])  # 客户端m的数据量
    payload = {"H": H_host, "d": delta_host, "n": n_local}
    ...
    if rank == 0:
        # ---- time: aggregate full-rank Hessian ----
        t3 = time.perf_counter()
        P32 = jnp.float32
        # 计算总数据量
        n_total = sum(item["n"] for item in gathered)
        H_sum = jnp.zeros((P, P), dtype=P32)
        b_sum = jnp.zeros((P,), dtype=P32)
        H_sum.block_until_ready(); b_sum.block_until_ready()
        # 按数据量加权聚合
        for item in gathered:
            H_i = device_put(jnp.asarray(item["H"], dtype=P32), DEV)
            d_i = device_put(jnp.asarray(item["d"], dtype=P32), DEV)
            n_m = float(item["n"])
            weight = n_m / n_total  # 权重 = N_m / N_total
            H_sum = H_sum + weight * H_i
            b_sum = b_sum + weight * (H_i @ d_i)
        H_sum.block_until_ready(); b_sum.block_until_ready()
        t_recon = time.perf_counter() - t3  # end aggregate

        # ---- time: solve linear system ----
    ...
    if rank == 0:
        ...
        time_this_round = float(
            tJJ_max + tLocal_max + tComm_max + t_recon + t_pinv + t_update + tBcast_max
        )
        ...
        mwfl_total = tJJ_max + t_recon + t_pinv + t_update
        ...
        with open(os.path.join(out_dir, "timings.csv"), "a") as f:
            f.write(
                f"{r},{tJJ_max:.6f},{tLocal_max:.6f},{tComm_max:.6f},"
    ...
    with open(local_metrics_path, "a") as f:
        f.write(f"{r},{g_norm:.10e},{r_norm:.10e},{LOCAL_EPOCHS}\n")
