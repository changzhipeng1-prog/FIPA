# -*- coding: utf-8 -*-
"""Distributed low-rank matrix-weighted FL with fixed local training steps (2D version)."""
...
parser.add_argument("--top_k", type=int, default=50, help="number of top eigenvalues to keep for low-rank approximation")
parser.add_argument("--time_budget", type=float, default=50.0)
parser.add_argument("--seed", type=int, default=0)
parser.add_argument("--out_root", type=str, default="Results_lowrank_2d")
...
if rank == 0:
    ...
    print(f"[config] low-rank: lr={LR}, local_epochs={LOCAL_EPOCHS}, step_size={STEP_SIZE}, top_k={TOP_K}, pinv_tol={PINV_TOL}")

# -------------------- 输出目录 --------------------
run_name = (
    f"lowrank_2d_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
    f"_epochs{LOCAL_EPOCHS}_step{STEP_SIZE}_k{TOP_K}_K{KX}x{KY}"
)
...
def eigs_top_k(JTJ, k):
    """
    Compute top k largest eigenvalues and corresponding eigenvectors.
    Returns (λ, Q) where λ are the top k eigenvalues and Q are the corresponding eigenvectors.
    """
    S = 0.5 * (JTJ + JTJ.T)  # Ensure symmetry
    w, V = jnp.linalg.eigh(S)
    # Select top k largest eigenvalues (last k elements)
...
for r in range(1, MAX_ROUNDS + 1):
    ...
    # ---- time: eigen decomposition and select top k ----
    ...
    if rank == 0:
        ...
        for item in gathered:
            ...
            if lam_i.size > 0:
                # Reconstruct H_i = Q_i @ diag(λ_i) @ Q_i^T for aggregation
                # More efficient: Q_i @ (lam_i[:, None] * Q_i.T) = Q_i @ diag(lam_i) @ Q_i.T
                H_i = Q_i @ (lam_i[:, None] * Q_i.T)
                H_sum = H_sum + H_i
                # Compute H_i @ d_i using low-rank representation (more efficient than H_i @ d_i)
