#!/bin/bash
# 写配置(JSON) -> 串行 sbatch 多个单节点作业 -> 作业内用 mpirun 起 N_CLIENTS 进程（FedAvg 2D + fixed local training）
set -euo pipefail
...
LOCAL_EPOCHS="${LOCAL_EPOCHS:-1000}"
...
# 可选
SEED="${SEED:-0}"
ALG_TAG="${ALG_TAG:-fedavg-2d}"

# 计算 N_CLIENTS（需要在OUT_ROOT之前计算）
N_CLIENTS=$((KX * KY))

# 输出路径包含客户端数量和网格信息
OUT_ROOT="${OUT_ROOT:-Results_fedavg_2d_K${KX}x${KY}}"
...
for EPOCHS in $EPOCHS_LIST; do
          ...
          CFG_FILE="$CFG_DIR/${ALG_TAG}_cfg_arch_${ARCH_TAG}_K${KX}x${KY}_D${DOMAIN_HASH}_epochs${EPOCHS}_rmax${MAX_ROUNDS}_tb${TIME_BUDGET}s_seed${SEED}_${STAMP}.json"
...
PY
          ...
          SBATCH_ARGS=(
            ...
            --mem="${MEM}"
            -t "${TIME_LIMIT}"
            -J "${ALG_TAG}_1n${N_CLIENTS}_K${KX}x${KY}_g${REQUEST_GPUS}_epochs${EPOCHS}_tb${TIME_BUDGET}s"
            -p "${PARTITION}"
            -o "slurm-%A.out"
          )
