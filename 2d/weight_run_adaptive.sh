...
mpirun -np "${N_CLIENTS}" \
  ...
  -x LAYERS -x DOMAIN -x KX -x KY -x ACT -x LR -x LOCAL_STEPS_CAP -x N_SAMPLES -x GRID_X -x GRID_Y \
  -x MAX_ROUNDS -x STEP_SIZE -x EIGVAL_THRESH -x TAU_COEF -x TIME_BUDGET -x SEED -x OUT_ROOT \
  -x PINV_TOL \
  bash -lc '
    ...
    exec python -u weight_distrib_adaptive.py \
      --layers "$LAYERS" \
      --domain "$DOMAIN" \
      --kx "$KX" \
      --ky "$KY" \
      --act "$ACT" \
      --lr "$LR" \
      --local_steps_cap "$LOCAL_STEPS_CAP" \
      --n_samples "$N_SAMPLES" \
      --grid_x "$GRID_X" \
      --grid_y "$GRID_Y" \
      --max_rounds "$MAX_ROUNDS" \
      --eigval_thresh "$EIGVAL_THRESH" \
      --pinv_tol "$PINV_TOL" \
      --step_size "$STEP_SIZE" \
      --tau_coef "$TAU_COEF" \
