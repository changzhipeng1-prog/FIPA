...
mpirun -np "${N_CLIENTS}" \
  ...
  -x LAYERS -x EDGES -x ACT -x N_FREQ -x LR -x LOCAL_EPOCHS -x N_SAMPLES -x GRID \
  -x MAX_ROUNDS -x STEP_SIZE -x TOP_K -x TIME_BUDGET -x OUT_ROOT \
  -x PINV_TOL \
  bash -lc '
    ...
    exec python -u weight_distrib_lowrank.py \
      --layers "$LAYERS" \
      --edges "$EDGES" \
      --act "$ACT" \
      --n_freq "$N_FREQ" \
