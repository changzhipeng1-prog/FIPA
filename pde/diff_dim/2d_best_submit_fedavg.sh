#!/bin/bash
# 2D维度最优参数配置 - LOCAL_EPOCHS=1000 (FedAvg, lowrank的10倍)
...
N_BOUNDARY="${N_BOUNDARY:-200}"  # boundary points per boundary face
LAMBDA_BC="${LAMBDA_BC:-100.0}" # boundary condition weight
GRID="${GRID:-1024}"
MAX_ROUNDS="${MAX_ROUNDS:-10000}"

# 关键超参：2D最优配置（FedAvg不需要STEP_SIZE和TOP_K）
...
echo "[plan] PINN FedAvg 2D最优配置: clients=$N_CLIENTS; request_gpus=$REQUEST_GPUS; collocation=$N_SAMPLES/client; boundary=$N_BOUNDARY; lambda_bc=$LAMBDA_BC; total_col=$TOTAL_SAMPLES; tb=${TIME_BUDGET}s"
...
SBATCH_ARGS=(
  ...
  --mem="${MEM}"
  -t "${TIME_LIMIT}"
  -J "${ALG_TAG}_2d_best_1n${N_CLIENTS}_g${REQUEST_GPUS}_epochs${LOCAL_EPOCHS}_bc${LAMBDA_BC}_tb${TIME_BUDGET}s"
  -p "${PARTITION}"
  -o "slurm-%A.out"
)
...
echo "[done] submitted 1 job (2D FedAvg最优配置)."
