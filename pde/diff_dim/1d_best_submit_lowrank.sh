...
echo "[plan] PINN 1D最优配置: clients=$N_CLIENTS; request_gpus=$REQUEST_GPUS; collocation=$N_SAMPLES/client; boundary=$N_BOUNDARY; lambda_bc=$LAMBDA_BC; total_col=$TOTAL_SAMPLES; tb=${TIME_BUDGET}s"
...
SBATCH_ARGS=(
  ...
  --mem="${MEM}"
  -t "${TIME_LIMIT}"
  -J "${ALG_TAG}_1d_best_1n${N_CLIENTS}_g${REQUEST_GPUS}_epochs${LOCAL_EPOCHS}_step${STEP_SIZE}_k${TOP_K}_bc${LAMBDA_BC}_tb${TIME_BUDGET}s"
  -p "${PARTITION}"
  -o "slurm-%A.out"
)
...
echo "[done] submitted 1 job (1D最优配置)."
