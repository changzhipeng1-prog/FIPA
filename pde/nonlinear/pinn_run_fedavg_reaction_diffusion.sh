...
mpirun -np "${N_CLIENTS}" \
  ...
  -x LAYERS -x EDGES -x ACT -x LR -x LOCAL_EPOCHS -x N_SAMPLES -x N_BOUNDARY -x LAMBDA_BC \
  -x GRID -x MAX_ROUNDS -x TIME_BUDGET -x OUT_ROOT \
  bash -c '
    ...
    exec python -u pinn_fedavg_reaction_diffusion.py \
      --layers "$LAYERS" \
      --edges "$EDGES" \
      --act "$ACT" \
      --lr "$LR" \
