#!/bin/bash
# 1D Reaction-Diffusion方程 FedAvg最优参数配置
...
# 1D最优参数（固定值）
N_SAMPLES="${N_SAMPLES:-800}"   # collocation points per client
...
# 可选
SEED="${SEED:-0}"
ALG_TAG="${ALG_TAG:-reaction_diffusion_fedavg}"
...
OUT_ROOT="${OUT_ROOT:-Results_reaction_diffusion_fedavg_N${N_CLIENTS}_bc${LAMBDA_BC}}"
...
echo "[plan] PINN FedAvg Reaction-Diffusion 1D最优配置: clients=$N_CLIENTS; request_gpus=$REQUEST_GPUS; collocation=$N_SAMPLES/client; boundary=$N_BOUNDARY; lambda_bc=$LAMBDA_BC; total_col=$TOTAL_SAMPLES; tb=${TIME_BUDGET}s"
...
SBATCH_ARGS=(
  ...
  --mem="${MEM}"
  -t "${TIME_LIMIT}"
  -J "${ALG_TAG}_1n${N_CLIENTS}_g${REQUEST_GPUS}_epochs${LOCAL_EPOCHS}_bc${LAMBDA_BC}_tb${TIME_BUDGET}s"
  -p "${PARTITION}"
  -o "slurm-%A.out"
)
...
sbatch "${SBATCH_ARGS[@]}" --export="$EXPORTS" pinn_run_fedavg_reaction_diffusion.sh

echo "[done] submitted 1 job (Reaction-Diffusion FedAvg最优配置)."
