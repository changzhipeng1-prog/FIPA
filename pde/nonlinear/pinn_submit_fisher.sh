...
EDGES="${EDGES:-[0,0.5,1.0]}"  # 2 clients for Fisher equation
ACT="${ACT:-tanh}"
LR="${LR:-1e-4}"
...
OUT_ROOT="${OUT_ROOT:-Results_fisher_N${N_CLIENTS}_bc${LAMBDA_BC}}"
...
PY
          ...
          sbatch "${SBATCH_ARGS[@]}" --export="$EXPORTS" pinn_run_fisher.sh
