# -*- coding: utf-8 -*-
"""Distributed FedAvg PINN for 1D Bratu equation with Dirichlet boundary conditions.
PDE: -d²u/dx² - λe^u = f(x)
...
if rank == 0:
    print(f"[config] PINN FedAvg (1D Bratu): lr={LR}, local_epochs={LOCAL_EPOCHS}")
    print(f"[config] lambda_bc={LAMBDA_BC}, lambda_bratu={LAMBDA_BRATU}, n_boundary={N_BOUNDARY}, layers={LAYERS}")

# -------------------- 输出目录 --------------------
run_name = (
    f"bratu_fedavg_1D_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
    f"_epochs{LOCAL_EPOCHS}_bc{LAMBDA_BC}_lam{LAMBDA_BRATU}"
)
