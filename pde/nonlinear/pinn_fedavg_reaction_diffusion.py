# -*- coding: utf-8 -*-
"""Distributed FedAvg PINN for 1D nonlinear reaction-diffusion equation with Dirichlet boundary conditions.
PDE: -d²u/dx² + u² = f(x)
Exact solution: u(x) = sin(2πx)  (on [0,1])
Source term: f(x) = 4π² sin(2πx) + sin²(2πx)
"""
...
if rank == 0:
    print(f"[config] PINN FedAvg (1D Reaction-Diffusion): lr={LR}, local_epochs={LOCAL_EPOCHS}")
    print(f"[config] lambda_bc={LAMBDA_BC}, n_boundary={N_BOUNDARY}, layers={LAYERS}")

# -------------------- 输出目录 --------------------
run_name = (
    f"reaction_diffusion_fedavg_1D_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
...
# -------------------- PDE Definition --------------------
# 1D Nonlinear reaction-diffusion: -d²u/dx² + u² = f(x)
# Exact solution: u(x) = sin(2πx)  (on [0,1])
# Source term: f(x) = 4π² sin(2πx) + sin²(2πx)
# Note: u''(x) = -4π² sin(2πx), so -u'' = 4π² sin(2πx)

def source_term(x):
    """1D source term: f(x) = 4π² sin(2πx) + sin²(2πx)"""
    # x shape: (..., 1) or (...,)
    x_flat = x.flatten() if x.ndim > 1 else x
    sin_2pi_x = jnp.sin(2.0 * jnp.pi * x_flat)
    return 4.0 * jnp.pi**2 * sin_2pi_x + sin_2pi_x**2

def exact_solution(x):
    """1D exact solution: u(x) = sin(2πx)"""
    # x shape: (..., 1) or (...,)
    x_flat = x.flatten() if x.ndim > 1 else x
    return jnp.sin(2.0 * jnp.pi * x_flat)
...
def pde_residual(params, x_col):
    """
    Compute PDE residual: -d²u/dx² + u² - f(x) for 1D nonlinear reaction-diffusion
