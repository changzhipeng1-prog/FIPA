# -*- coding: utf-8 -*-
"""Distributed FedAvg PINN for 1D Allen-Cahn equation with Dirichlet boundary conditions.
PDE: -d²u/dx² + u³ - u = f(x)
Exact solution: u(x) = sin(πx)
Source term: f(x) = π² sin(πx) + sin³(πx) - sin(πx)
"""
...
if rank == 0:
    print(f"[config] PINN FedAvg (1D Allen-Cahn): lr={LR}, local_epochs={LOCAL_EPOCHS}")
    print(f"[config] lambda_bc={LAMBDA_BC}, n_boundary={N_BOUNDARY}, layers={LAYERS}")

# -------------------- 输出目录 --------------------
run_name = (
    f"allen_cahn_fedavg_1D_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
...
# -------------------- PDE Definition --------------------
# 1D Allen-Cahn equation: -d²u/dx² + u³ - u = f(x)
# Exact solution: u(x) = sin(πx)
# Source term: f(x) = π² sin(πx) + sin³(πx) - sin(πx)

def source_term(x):
    """1D source term: f(x) = π² sin(πx) + sin³(πx) - sin(πx)"""
    # x shape: (..., 1) or (...,)
    x_flat = x.flatten() if x.ndim > 1 else x
    sin_pi_x = jnp.sin(jnp.pi * x_flat)
    return jnp.pi**2 * sin_pi_x + sin_pi_x**3 - sin_pi_x

def exact_solution(x):
    """1D exact solution: u(x) = sin(πx)"""
    # x shape: (..., 1) or (...,)
    x_flat = x.flatten() if x.ndim > 1 else x
    return jnp.sin(jnp.pi * x_flat)
...
def pde_residual(params, x_col):
    """
    Compute PDE residual: -d²u/dx² + u³ - u - f(x) for 1D Allen-Cahn equation
    ...
    def pde_operator(x):
        """Compute PDE operator: -d²u/dx² + u³ - u"""
        u = u_fn(x)
