...
EDGES="${EDGES:-[0,0.5,1.0]}"  # 2 clients for reaction-diffusion equation
ACT="${ACT:-tanh}"
LR="${LR:-1e-4}"
...
# 可选
SEED="${SEED:-0}"
ALG_TAG="${ALG_TAG:-reaction_diffusion}"
...
OUT_ROOT="${OUT_ROOT:-Results_reaction_diffusion_N${N_CLIENTS}_bc${LAMBDA_BC}}"
...
PY
          ...
          sbatch "${SBATCH_ARGS[@]}" --export="$EXPORTS" pinn_run_reaction_diffusion.sh
