# -*- coding: utf-8 -*-
"""Distributed low-rank PINN for 1D Bratu equation with Dirichlet boundary conditions.
PDE: -d²u/dx² - λe^u = f(x)
Exact solution: u(x) = x(1-x)  (on [0,1])
Source term: f(x) = 2 - λe^(x(1-x))
"""
...
parser.add_argument("--lambda_bratu", type=float, default=1.0, help="Bratu equation coefficient")
...
LAMBDA_BRATU = float(args.lambda_bratu)
...
if rank == 0:
    print(f"[config] PINN low-rank (1D Bratu): lr={LR}, local_epochs={LOCAL_EPOCHS}, step_size={STEP_SIZE}")
    print(f"[config] top_k={TOP_K}, lambda_bc={LAMBDA_BC}, lambda_bratu={LAMBDA_BRATU}, n_boundary={N_BOUNDARY}, layers={LAYERS}")

# -------------------- 输出目录 --------------------
run_name = (
    f"bratu_1D_tb-{int(TIME_BUDGET)}s_mr-{MAX_ROUNDS}"
    f"_epochs{LOCAL_EPOCHS}_step{STEP_SIZE}_k{TOP_K}_bc{LAMBDA_BC}_lam{LAMBDA_BRATU}"
)
...
# -------------------- PDE Definition --------------------
# 1D Bratu equation: -d²u/dx² - λe^u = f(x)
# Exact solution: u(x) = x(1-x)  (on [0,1])
# Source term: f(x) = 2 - λe^(x(1-x))
# Note: u''(x) = -2, so -u'' = 2

def source_term(x):
    """1D source term: f(x) = 2 - λe^(x(1-x))"""
    # x shape: (..., 1) or (...,)
    x_flat = x.flatten() if x.ndim > 1 else x
    u_val = x_flat * (1.0 - x_flat)  # u(x) = x(1-x)
    return 2.0 - LAMBDA_BRATU * jnp.exp(u_val)

def exact_solution(x):
    """1D exact solution: u(x) = x(1-x)"""
    # x shape: (..., 1) or (...,)
    x_flat = x.flatten() if x.ndim > 1 else x
    return x_flat * (1.0 - x_flat)
...
def pde_residual(params, x_col):
    """
    Compute PDE residual: -d²u/dx² - λe^u - f(x) for 1D Bratu equation
    ...
    def pde_operator(x):
        """Compute PDE operator: -d²u/dx² - λe^u"""
        u = u_fn(x)
        u_xx = jax.grad(jax.grad(u_fn))(x)
        return -u_xx - LAMBDA_BRATU * jnp.exp(u)
    
    # Compute PDE operator for each collocation point
    # x_col shape: (n_col, 1), flatten to (n_col,) for vmap
    x_col_flat = x_col.reshape(-1)  # shape: (n_col,)
    pde_val = jax.vmap(pde_operator)(x_col_flat)  # shape: (n_col,)
