...
EDGES="${EDGES:-[0,0.5,1.0]}"  # 2 clients for Bratu equation
ACT="${ACT:-tanh}"
LR="${LR:-1e-4}"
...
OUT_ROOT="${OUT_ROOT:-Results_bratu_N${N_CLIENTS}_bc${LAMBDA_BC}}"
...
echo "[plan] PINN single-node: clients=$N_CLIENTS; request_gpus=$REQUEST_GPUS; collocation=$N_SAMPLES/client; boundary=$N_BOUNDARY; lambda_bc=$LAMBDA_BC; lambda_bratu=$LAMBDA_BRATU; total_col=$TOTAL_SAMPLES; tb=${TIME_BUDGET}s"
...
for EPOCHS in $EPOCHS_LIST; do
  for STEP in $STEP_LIST; do
    for K in $TOP_K_LIST; do
          ...
          CFG_FILE="$CFG_DIR/${ALG_TAG}_cfg_arch_${ARCH_TAG}_N${N_CLIENTS}_E${EDGE_HASH}_bc${LAMBDA_BC}_lam${LAMBDA_BRATU}_epochs${EPOCHS}_step${STEP}_k${K}_rmax${MAX_ROUNDS}_tb${TIME_BUDGET}s_seed${SEED}_${STAMP}.json"
...
PY
          ...
          SBATCH_ARGS=(
            ...
            --mem="${MEM}"
            -t "${TIME_LIMIT}"
            -J "${ALG_TAG}_1n${N_CLIENTS}_g${REQUEST_GPUS}_epochs${EPOCHS}_step${STEP}_k${K}_bc${LAMBDA_BC}_lam${LAMBDA_BRATU}_tb${TIME_BUDGET}s"
            -p "${PARTITION}"
            -o "slurm-%A.out"
          )
          ...
          sbatch "${SBATCH_ARGS[@]}" --export="$EXPORTS" pinn_run_bratu.sh
