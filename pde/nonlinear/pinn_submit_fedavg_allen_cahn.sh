...
# 可选
SEED="${SEED:-0}"
ALG_TAG="${ALG_TAG:-allen_cahn_fedavg}"
...
OUT_ROOT="${OUT_ROOT:-Results_allen_cahn_fedavg_N${N_CLIENTS}_bc${LAMBDA_BC}}"
...
echo "[plan] PINN FedAvg Allen-Cahn 1D最优配置: clients=$N_CLIENTS; request_gpus=$REQUEST_GPUS; collocation=$N_SAMPLES/client; boundary=$N_BOUNDARY; lambda_bc=$LAMBDA_BC; total_col=$TOTAL_SAMPLES; tb=${TIME_BUDGET}s"
...
sbatch "${SBATCH_ARGS[@]}" --export="$EXPORTS" pinn_run_fedavg_allen_cahn.sh

echo "[done] submitted 1 job (Allen-Cahn FedAvg最优配置)."
